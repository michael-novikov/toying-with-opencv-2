...
const int DATA_NUM = 4'000;
const int VAR_COUNT = 45 * 45;
...
static int build_rtrees_classifier(const string& dir_path, const string& filename_to_save, const string& filename_to_load) {
	Mat data;
	Mat responses;
	Mat var_type;
	Mat sample_idx;

	bool ok = loadImages(dir_path, DATA_NUM, VAR_COUNT, &data, &responses);
	...
	if (!filename_to_load.empty())
	{
		...
		forest.load(filename_to_load.c_str());
		ntrain_samples = 0;
	...
	else
	{
		...
		// 1. create type mask
		var_type.create(data.cols + 1, 1, CV_8UC1);
		cvSet(&var_type, cvScalarAll(CV_VAR_ORDERED));
		cvSetReal1D(&var_type, data.cols, CV_VAR_CATEGORICAL);

		// 2. create sample_idx
		sample_idx.create(1, nsamples_all, CV_8UC1);
		{
			CvMat mat;
	...
	if (!filename_to_save.empty())
		forest.save(filename_to_save.c_str());
...
//static int build_boost_classifier(char* data_filename, char* filename_to_save, char* filename_to_load)
//{
...
int main(int argc, char *argv[])
{
	...
	string default_data_filename = "./images";
	string data_filename = default_data_filename;
	int method = 0;
	...
	if (i < argc ||
		(method == 0 ?
			build_rtrees_classifier(data_filename, filename_to_save, filename_to_load) : -1) < 0)
			/*method == 1 ?
